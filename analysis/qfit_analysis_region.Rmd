...
input <- read_csv("../output/input.csv", col_types = cols(.default = col_character())) %>% 
  mutate(
    across(c(matches("^(qfit|ft.*|crc.*)_date$"), colonoscopy_sus, flexi_sig_sus, colonoscopy_primary_care, flexi_sig_primary_care), lubridate::ymd),
...
```{r}
primary_data %>%
  count(region, primary_care = crc_diagnosis, seconday_care = !is.na(crc_admission_date)) %>%
  knitr::kable()
```